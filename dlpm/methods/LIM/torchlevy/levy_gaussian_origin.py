...
def levy_gaussian_score_origin(alpha: float, x: torch.Tensor, sigma1s: Union[list, torch.Tensor],
                        sigma2s: Union[list, torch.Tensor], beta=0, t0=30, Fs=100, mode='fraction'):
    score = torch.zeros_like(x)

    for i, (s1, s2) in enumerate(zip(sigma1s, sigma2s)):
        score_dict = _get_score_dict_cft(alpha, s1, s2, beta, t0, Fs, mode=mode)
